...
class Model(nn.Module):
    ...
    def __init__(self, config, channels=3, num_classes=None, fuse_ab=False, distill_ns=False):  # model, input channels, number of classes
        ...
        self.backbone, self.neck, self.detect1, self.detect2 = build_network(config, channels, num_classes, num_layers, fuse_ab=fuse_ab, distill_ns=distill_ns)

        # Init Detect head
        self.stride = self.detect1.stride
        self.detect1.initialize_biases()
        self.detect2.initialize_biases()
    ...
    def forward(self, x, mode=3):
        ...
        if not export_mode:
            ...
            x1 = self.detect1(x)
            x2 = self.detect2(x)
            out = {'output1': x1, 'output2': x2}
        return out if export_mode is True else [x, featmaps]

    def _apply(self, fn):
        self = super()._apply(fn)
        self.detect1.stride = fn(self.detect1.stride)
        self.detect1.grid = list(map(fn, self.detect1.grid))
        self.detect2.stride = fn(self.detect2.stride)
        self.detect2.grid = list(map(fn, self.detect2.grid))
        return self
...
def build_network(config, channels, num_classes, num_layers, fuse_ab=False, distill_ns=False):
    ...
    if distill_ns:
        ...
        head_layers1 = build_effidehead_layer(channels_list, 1, num_classes, reg_max=reg_max, num_layers=num_layers)
        head1 = Detect(num_classes, num_layers, head_layers=head_layers1, use_dfl=use_dfl)
        head_layers2 = build_effidehead_layer(channels_list, 1, num_classes, reg_max=reg_max, num_layers=num_layers)
        head2 = Detect(num_classes, num_layers, head_layers=head_layers2, use_dfl=use_dfl)

    return backbone, neck, head1, head2
