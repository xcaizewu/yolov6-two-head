...
class DetectBackend(nn.Module):
    ...
    def forward(self, im, mode=3, val=False):
        y, _ = self.model(im, mode=mode)
        if isinstance(y, np.ndarray):
            y = torch.tensor(y, device=self.device)
        return y
#onnx模型加载与推理
import onnxruntime as ort
class Detectonnx(nn.Module):
    def __init__(self, weights='yolov6s.onnx', device=None, dnn=True):
        super().__init__()
        assert isinstance(weights, str) and Path(weights).suffix == '.onnx', f'{Path(weights).suffix} format is not supported.'
        model = ort.InferenceSession(weights,providers = ["CPUExecutionProvider"]) #MacavxExecutionProvider
        self.__dict__.update(locals())
    def forward(self, im, val=False):
        ort_inputs = {self.model.get_inputs()[0].name: im}
        y = self.model.run(None,ort_inputs)
        if isinstance(y, np.ndarray):
            y = torch.tensor(y, device=self.device)
        return y
