...
class TrainValDataset(Dataset):
    ...
    def __init__(
        ...
        # print('66666666666666666666666666666', self.task, self.rect)
        if self.rect:  # 只有验证集才会进来
    ...
    def __getitem__(self, index):
        ...
        """
        target_shape = (  # 训练集self.rect为False，直接返回self.img_size，而验证集时，self.rect为True,则走的self.batch_shapes[self.batch_indices[index]
                (self.target_height, self.target_width) if self.specific_shape else
                self.batch_shapes[self.batch_indices[index]] if self.rect
                else self.img_size
                )
        # print('11111111111111111', self.specific_shape, self.rect, target_shape, self.augment, random.random() < self.hyp["mosaic"], self.task)
        # Mosaic Augmentation
        if self.augment and random.random() < self.hyp["mosaic"]:  # 训练集走这里
            img, labels = self.get_mosaic(index, target_shape)  # TODO: 这里通过马赛克增强将4张图片拼接成(640， 640)
            # print(f'oooo{img.shape}')
            shapes = None
            ...
            if self.hyp and "shrink_size" in self.hyp:
                ...
                img, (h0, w0), (h, w) = self.load_image(index)  # 计算ratio， 按比例resize

            # print(f'org shape: {(h0, w0)} after shape: {(h, w)} target shape: {target_shape}')
            # letterbox
            img, ratio, pad = letterbox(img, target_shape, auto=False, scaleup=self.augment)  # batch_shape中的pad为0.5, 所以上一步resize后，这步主要是用黑色填充扩展成batch_shapes, 在训练时没走这个函数，直接马赛克后shape为（640， 640）
            # print(f'letterbox img shape: {img.shape} ratio: {ratio}')
        ...
        # print(f'kkkkkk {img.shape}')  # 训练集此时的shape都为(640, 640) TODO: 但是验证集不进行马赛克增强，所以shape可变，head1最多的为(384, 672)，head2比例就多了，图片的shape种类多造成的
        return torch.from_numpy(img), labels_out, self.img_paths[index], shapes
 
    def load_image(self, index, shrink_size=None):
        """Load image.
        该函数使用cv2（即OpenCV库中的一个模块）来加载图像，并保持比例地将原始图像调整到目标尺寸（img_size）。下面是这个过程的详细说明：
            加载图像：使用cv2库中的函数（通常是cv2.imread）来读取图像文件。
            调整图像尺寸：将读取到的原始图像调整到指定的目标尺寸img_size。这个调整过程会保持图像的宽高比，以避免图像失真。
            保持比例：在调整图像大小时，不会简单地拉伸或压缩图像，而是通过计算原始图像和目标尺寸之间的比例，来等比例地调整图像的宽度和高度。
        ...
        if self.cache_ram and self.imgs[index] is not None:
            ...
            # print(f'train {ratio} {h0, w0} {im.shape[:2]}')  train 0.8888888888888888 (404, 720) (359, 640) train 0.3333333333333333 (1080, 1920) (360, 640)
    ...
    def get_imgs_labels(self, img_dirs):
        ...
        # check images
        if self.check_images and self.main_process: # False and True
            img_info = {}
    ...
    def sort_files_shapes(self):
        '''w / h 计算ratio, 根据ratio重新排序，将相应ratio的数据为一个batch，并计算这个batch的输入shape是多少，只有验证集会进这个函数'''
        batch_num = self.batch_indices[-1] + 1
        s = self.shapes  # [height, width]
        ar = s[:, 1] / s[:, 0]  # aspect ratio w / h
        irect = ar.argsort()
        ...
        for i in range(batch_num):
            ...
            # print(shapes[i], self.img_size, self.stride, self.pad, np.ceil(np.array(shapes[i]) * self.img_size / self.stride + self.pad).astype(
            #     np.int_
            # ))
