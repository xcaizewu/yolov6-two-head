...
from yolov6.layers.common import Detectonnx
...
class Inferer:
    def __init__(self, source, webcam, webcam_addr, weights, device, yaml, img_size, half):
        ...
        # 加一个判断，onnx模型执行另外一个加载函数
        if weights[-4:] == 'onnx':
            self.model = Detectonnx(weights)
            self.stride = 32
        else:
    ...
    def infer(self, conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf, weights , view_img=True):
        ...
        for img_src, img_path, vid_cap in tqdm(self.files):
            ...
            t1 = time.time()
            pred_results = self.model(img, mode=1)
            if weights[-4:] == 'onnx':
                pred_results_array = np.array(pred_results)
                pred_results = torch.tensor(pred_results_array).to(self.device)
    ...
    def process_image(img_src, img_size, stride, half):
        ...
        print(f'letterbox: org_shape {img_src.shape} img_size {img_size} after shape {image.shape}')
        # Convert
    ...
    @staticmethod
    def process_image_onnx(img_src, img_size, stride, half):
        '''Process image before image inference.'''
        # image = letterbox(img_src, img_size, stride=stride)[0]
