...
class Trainer:
    def __init__(self, args, cfg, device):
        ...
        self.train_head1_loader, self.val_head1_loader, self.train_head2_loader, self.val_head2_loader = self.get_data_loader(self.args, self.cfg, self.data_dict)

        self.model = self.parallel_model(args, model, device)
        self.model.nc, self.model.names = self.data_dict['nc'], self.data_dict['names']

        self.max_stepnum1 = len(self.train_head1_loader)
        self.max_stepnum2 = len(self.train_head2_loader)
    ...
    def train_one_epoch(self, epoch_num):
        try:
            for self.step, self.batch_data in self.pbar1:
                self.train_in_steps1(epoch_num, self.step)
                self.print_details()
            for self.step, self.batch_data in self.pbar2:
                self.train_in_steps2(epoch_num, self.step, mode=2)
                self.print_details(mode=2)
        except Exception as _:
    ...
    def train_in_steps1(self, epoch_num, step_num, mode=1):
        images, targets = self.prepro_data(self.batch_data, self.device)
        # print('mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm', images.shape)
        ...
        if self.write_trainbatch_tb and self.main_process and self.step == 0:
            ...
            write_tbimg(self.tblogger, self.vis_train_batch, self.step + self.max_stepnum1 * self.epoch, type='train')

        # forward
        with amp.autocast('cuda', enabled=self.device != 'cpu'):
            _, _, batch_height, batch_width = images.shape
            preds, s_featmaps = self.model(images, mode=mode)
            if self.args.distill:
                with torch.no_grad():
                ...
                total_loss, loss_items = self.compute_loss1(preds, targets, epoch_num, step_num,
    ...
    def train_in_steps2(self, epoch_num, step_num, mode=1):
        images, targets = self.prepro_data(self.batch_data, self.device)
        # print('vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv', images.shape)
        # plot train_batch and save to tensorboard once an epoch
        if self.write_trainbatch_tb and self.main_process and self.step == 0:
            self.plot_train_batch(images, targets)
            write_tbimg(self.tblogger, self.vis_train_batch, self.step + self.max_stepnum2 * self.epoch, type='train')
        ...
        with amp.autocast('cuda', enabled=self.device != 'cpu'):
            ...
            if self.args.distill:
                ...
                total_loss, loss_items = self.compute_loss2(preds, targets, epoch_num, step_num,
                                                            batch_height, batch_width) # YOLOv6_af
            if self.rank != -1:
                total_loss *= self.world_size
        # backward
    ...
    def eval_model(self):
        if not hasattr(self.cfg, "eval_params"):
            self.data_dict['val'] = self.data_dict['val_head1']
            results1, vis_outputs1, vis_paths1 = eval.run(self.data_dict,
                            batch_size=self.batch_size // self.world_size * 2,
                            img_size=self.img_size,
                            model=self.ema.ema if self.args.calib is False else self.model,
                            conf_thres=0.03,
                            dataloader=self.val_head1_loader,
            ...
            LOGGER.info(f"Epoch: {self.epoch} | mAP@0.5: {results1[0]} | mAP@0.50:0.95: {results1[1]}")
            self.data_dict['val'] = self.data_dict['val_head2']
            results2, vis_outputs2, vis_paths2 = eval.run(self.data_dict,
                                                       batch_size=self.batch_size // self.world_size * 2,
                                                       img_size=self.img_size,
                                                       model=self.ema.ema if self.args.calib is False else self.model,
                                                       conf_thres=0.03,
                                                       dataloader=self.val_head2_loader,
                                                       save_dir=self.save_dir,
                                                       task='train',
                                                       specific_shape=self.specific_shape,
                                                       height=self.height,
            ...
            LOGGER.info(f"Epoch: {self.epoch} | mAP@0.5: {results2[0]} | mAP@0.50:0.95: {results2[1]}")
        else:
        ...
        self.evaluate_results = ((results1[0] + results2[0]) / 2, (results1[1] + results2[1]) / 2)
        # plot validation predictions
        self.plot_val_pred(vis_outputs1, vis_paths1)
        self.plot_val_pred(vis_outputs2, vis_paths2)


    def before_train_loop(self):
        LOGGER.info('Training start...')
        self.start_time = time.time()
        self.warmup_stepnum = max(round(self.cfg.solver.warmup_epochs * self.max_stepnum2), 1000) if self.args.quant is False else 0
        ...
        self.compute_loss1 = ComputeLoss(num_classes=self.data_dict['nc'],
        ...
        self.compute_loss2 = ComputeLoss(num_classes=self.data_dict['nc'],
                                        ori_img_size=self.img_size,
                                        warmup_epoch=self.cfg.model.head.atss_warmup_epoch,
                                        use_dfl=self.cfg.model.head.use_dfl,
                                        reg_max=self.cfg.model.head.reg_max,
    ...
    def before_epoch(self):
        ...
        self.model.train()
        if self.rank != -1:
            self.train_head1_loader.sampler.set_epoch(self.epoch)
        self.mean_loss = torch.zeros(self.loss_num, device=self.device)
        self.optimizer.zero_grad()

        LOGGER.info(('\n' + '%10s' * (self.loss_num + 2)) % (*self.loss_info,))
        self.pbar1 = enumerate(self.train_head1_loader)
        self.pbar2 = enumerate(self.train_head2_loader)
        if self.main_process:
            self.pbar1 = tqdm(self.pbar1, total=self.max_stepnum1, ncols=NCOLS,
                              bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}')
            self.pbar2 = tqdm(self.pbar2, total=self.max_stepnum2, ncols=NCOLS, bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}')

    # Print loss after each steps
    def print_details(self, mode=1):
        if self.main_process:
            self.mean_loss = (self.mean_loss * self.step + self.loss_items) / (self.step + 1)
            if mode == 1:
                self.pbar1.set_description(('%10s' + ' %10.4g' + '%10.4g' * self.loss_num) % (f'{self.epoch}/{self.max_epoch - 1}', \
                                                                    self.scheduler.get_last_lr()[0], *(self.mean_loss)))
            else:
                self.pbar2.set_description(
    ...
    def update_optimizer(self):
        curr_step = self.step + self.max_stepnum2 * self.epoch
    ...
    def get_data_loader(args, cfg, data_dict):
        train_head1_path, val_head1_path = data_dict['train_head1'], data_dict['val_head1']
        train_head2_path, val_head2_path = data_dict['train_head2'], data_dict['val_head2']
        ...
        # create train dataexpectedloader
        print('2222222222222222', args)
        train_head1_loader = create_dataloader(train_head1_path, args.img_size, args.batch_size // args.world_size, grid_size,
        ...
        print('33333333333333333', args)
        train_head2_loader = create_dataloader(train_head2_path, args.img_size, args.batch_size // args.world_size, grid_size,
                                         hyp=dict(cfg.data_aug), augment=True, rect=args.rect, rank=args.local_rank,
                                         workers=args.workers, shuffle=True, check_images=args.check_images,
                                         check_labels=args.check_labels, data_dict=data_dict, task='train',
                                         specific_shape=args.specific_shape, height=args.height, width=args.width,
        ...
        if args.rank in [-1, 0]:
            ...
            val_head1_loader = create_dataloader(val_head1_path, args.img_size, args.batch_size // args.world_size * 2, grid_size,
                                           ...
                                           specific_shape=args.specific_shape, height=args.height, width=args.width,
                                           cache_ram=args.cache_ram)[0]
            val_head2_loader = create_dataloader(val_head2_path, args.img_size, args.batch_size // args.world_size * 2, grid_size,
                                           hyp=dict(cfg.data_aug), rect=True, rank=-1, pad=0.5,
                                           workers=args.workers, check_images=args.check_images,
                                           check_labels=args.check_labels, data_dict=data_dict, task='val',
                                           specific_shape=args.specific_shape, height=args.height, width=args.width,
                                           cache_ram=args.cache_ram)[0]
        return train_head1_loader, val_head1_loader, train_head2_loader, val_head2_loader
