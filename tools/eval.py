...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--weights', type=str, default='/home/user/8T/ZY/code/zc/YOLOv6/runs/train/exp3/weights/best_ckpt.pt', help='model.pt path(s)')
...
def run(data,
    ...
    model = val.init_model(model, weights, task)  # model有值， weights为None
    dataloader = val.init_data(dataloader, task)

    # eval
    model.eval()
    pred_result, vis_outputs, vis_paths = val.predict_model(model, dataloader, task, mode)
