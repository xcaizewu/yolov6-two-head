...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--weights', type=str, default='/home/user/8T/ZY/code/zc/YOLOv6/runs/train/exp3/weights/best_ckpt.pt', help='model path(s) for inference.')  #
    parser.add_argument('--source', type=str, default='/home/user/double_test/', help='the source path, e.g. image-file/dir.')  # /home/user/caizewu/project/中车/中车数据/高铁项目/训练数据/data/images/test/
    parser.add_argument('--webcam', default=False, action='store_true', help='whether to use webcam.')
    parser.add_argument('--webcam-addr', type=str, default='', help='the web camera address, local camera or rtsp address.')
    ...
    parser.add_argument('--conf-thres', type=float, default=0.3, help='confidence threshold for inference.')
...
def run(weights=osp.join(ROOT, 'yolov6s.pt'),
    ...
    inferer.infer(conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, not not_save_img, hide_labels, hide_conf, weights, view_img)
